...
train_slice = (slice(15, 23), slice(0, 600), slice(600, 1200))
test_slice = (slice(15, 23), slice(600, 1200), slice(600, 1200))
...
def volume_seg_bpm(plot=False):
    ...
    if plot:
        ...
        ax[1, 0].set_xlabel('False merges (in bits)')
        ax[1, 0].set_ylabel('False splits (in bits)')
        ax[1, 0].set_title("Split VI")
        ax[1, 1].set_title("Cumulative VI")
        ax[1, 1].set_xlabel('Merge Threshold')
        ax[1, 1].set_ylabel('VI (in bits)')
        ax[1, 1].plot(split_vi_array_bpm.sum(axis=1))
        ax[1, 2].imshow(raw_training[3,...])
        plt.show()
    return np.min(split_vi_array_bpm.sum(axis=1))


def volume_seg_non_bpm(plot=False):
    ...
    if plot:
        ...
        ax[1, 2].imshow(raw_training[3, ...])
        plt.show()
    return np.min(split_vi_array.sum(axis=1))
...
if __name__ == '__main__':
    ...
    minimum_bpm = volume_seg_bpm(plot=True)
    #minimum_non_bpm = volume_seg_non_bpm(plot=True)
    #print(f"The minimum Cumulative VI of the non_bpm segmentation is {minimum_non_bpm}\n")
    print(f"The minimum Cumulative VI of the bpm segmentation is {minimum_bpm}\n")
